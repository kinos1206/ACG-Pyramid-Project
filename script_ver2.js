...
function animate() {
    ...
    // night sky
    let skyStarOpacity = getStarOpacity(angle);
    // stars rotation
    ...
    starq.setFromAxisAngle(earthAxis, 0.02);
...
function updateFog() {
    if (fogActive) {
        ...
        if (Math.random() < 0.01) { // Random chance to start fog
            fogActive = true;
            fogTimer = Math.random() * 120 + 60; // Random duration between 1-3 minutes
            scene.fog.density = fogMaxDensity;
        }
    }
}
...
function getStarOpacity(angle){
    let opacity = (-Math.sin(angle) + 1) * 0.8;
